...
def create_dataset(dataset, fold=0, cross_val_flag=False):
    # load dataset
    ...
    if not cross_val_flag:  # all train entries
        fold = -1
        train_folds = [ee for e in train_fold_origin for ee in e]
    ...
    for opt in opts:
        ...
        elif opt == 'valid' and cross_val_flag:
            rows, cols = np.where(np.isnan(affinity) == False)
    ...
    # if no cross-validation, the train data csv will be saved as fold_0
