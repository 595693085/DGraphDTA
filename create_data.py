...
def target_to_feature(target_key, target_sequence, dataset):
...
def target_to_graph(target_key, target_sequence, dataset):
    target_edge_index = []
    ...
    target_feature = target_to_feature(target_key, target_sequence, dataset)
...
def create_dataset(dataset, fold=0):
    ...
    fpath = 'data/' + dataset + '/'
    train_fold_origin = json.load(open(fpath + "folds/train_fold_setting1.txt"))
    ...
    test_path = 'data/' + dataset + '/aln'
    test_path2 = 'data/' + dataset + '/pconsc4'
    test_list = []
    test_list2 = []
    for key in proteins:
        test_list.append(os.path.join(test_path, key + ".aln"))
        test_list2.append(os.path.join(test_path2, key + ".npy"))
    ...
    print('fold', fold)
    print('train_fold:', len(train_fold), 'effective train_fold', valid_train_count)
    ...
    seq_voc = "ABCDEFGHIKLMNOPQRSTUVWXYZ"  # no using, left from GraphDTA
    seq_dict = {v: i for i, v in enumerate(seq_voc)}  # no using, left from GraphDTA
    ...
    for smile in compound_iso_smiles:
        ...
        smile_graph[smile] = g
    # print(smile_graph["CN1CCN(C(=O)c2cc3cc(Cl)ccc3[nH]2)CC1"])
    ...
    for key in target_key:
        if not valid_target(key, dataset):
            continue
        g = target_to_graph(key, proteins[key], dataset)
        target_graph[key] = g
    ...
    # create dataset in PyTorch Geometric format, and valid_dataset is corresponding to train_dataset,

    df_train_fold = pd.read_csv('data/' + dataset + '_' + 'fold_' + str(fold) + '_' + 'train' + '.csv')
    train_drugs, train_prot_seqs, train_prot_keys, train_Y = list(df_train_fold['compound_iso_smiles']), list(
        df_train_fold['target_sequence']), list(df_train_fold['target_key']), list(df_train_fold['affinity'])
    train_prot_seqs = [seq_cat(t, seq_dict) for t in train_prot_seqs]
    train_drugs, train_prot_seqs, train_prot_keys, train_Y = np.asarray(train_drugs), np.asarray(
        train_prot_seqs), np.asarray(train_prot_keys), np.asarray(train_Y)
    train_dataset = TestbedDataset(root='data', dataset=dataset + '_' + 'train', xd=train_drugs, xt=train_prot_seqs,
    ...
    valid_drugs, valid_prot_seqs, valid_prots_keys, valid_Y = list(df_valid_fold['compound_iso_smiles']), list(
        df_valid_fold['target_sequence']), list(df_valid_fold['target_key']), list(df_valid_fold['affinity'])
    valid_prot_seqs = [seq_cat(t, seq_dict) for t in valid_prot_seqs]
    valid_drugs, valid_prot_seqs, valid_prots_keys, valid_Y = np.asarray(valid_drugs), np.asarray(
        valid_prot_seqs), np.asarray(valid_prots_keys), np.asarray(valid_Y)
    valid_dataset = TestbedDataset(root='data', dataset=dataset + '_' + 'train', xd=valid_drugs, xt=valid_prot_seqs,
                                   target_key=valid_prots_keys, y=valid_Y, smile_graph=smile_graph,
                                   target_graph=target_graph)

    df_test = pd.read_csv('data/' + dataset + '_test.csv')
    test_drugs, test_prots, test_prot_keys, test_Y = list(df_test['compound_iso_smiles']), list(
        df_test['target_sequence']), list(df_test['target_key']), list(df_test['affinity'])
    test_prots = [seq_cat(t, seq_dict) for t in test_prots]
    test_drugs, test_prots, test_prot_keys, test_Y = np.asarray(test_drugs), np.asarray(test_prots), np.asarray(
        test_prot_keys), np.asarray(test_Y)
    test_dataset = TestbedDataset(root='data', dataset=dataset + '_test', xd=test_drugs, y=test_Y, xt=test_prots,
